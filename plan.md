...
1. 在標注界面多一行工具列包含”箭頭”、”選取工具”、”刪除”、”全選” 用icon表示 
2. 在選取完多個框之後會在框的上方多出一個長條選項欄位，有四個選項可以選（TL->TR->BL->BR / TL->BL->TR->BR / TR->TL->BR->BL / TR->BR->TL->BL）並多一個圓形驚嘆號代表說明按鈕，點擊後會跳出一個懸浮視窗，裡面有每一個選項示意圖(幫我留圖的位置，我會自己補上)，說明頁面可以關掉，上方長條點擊選項後跳出處理的loading前端，處理完後會將目前選取的框根據選的選項調整順序。
這樣設計可以多一個部分閱讀順序校正的作用，因為可以選取部分的框做閱讀順序校正沒有強制要全部框一起排序，。
3. 新增手動調整框編號的方法，每一個框可以選擇要插入哪個編號(“請選擇插入位置”)，其餘編號自動向後調整。
